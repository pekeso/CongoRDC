...
TestCashflowRDC.prototype.testVerifyMethods = function() {
   ...
   Test.assertIsEqual(Number(calculate_FA(banDoc, startDate, endDate)), Number("118056400.00"));
   ...
   Test.assertIsEqual(Number(calculate_FD(banDoc, startDate, endDate)), Number("245361680.00"));
   Test.assertIsEqual(Number(calculate_FE(banDoc, startDate, endDate)), Number("284035880.00"));
   Test.assertIsEqual(Number(calculate_FF(banDoc, startDate, endDate)), Number("295800.00"));
   Test.assertIsEqual(Number(calculate_FG(banDoc, startDate, endDate)), Number("23409320.00"));
   ...
   Test.assertIsEqual(Number(calculate_FL(banDoc, startDate, endDate)), Number("15000000.00"));
   ...
   Test.assertIsEqual(Number(tot_BF), Number("541152560.00"));
   ...
   var tot_ZB = calculate_tot_ZB(
      ...
      calculate_FD(banDoc, startDate, endDate),
      calculate_FE(banDoc, startDate, endDate));
   Test.assertIsEqual(Number(tot_ZB), Number("144975600.00"));
   ...
   Test.assertIsEqual(Number(tot_ZC), Number("-43905120.00"));
   ...
   Test.assertIsEqual(Number(tot_ZD), Number("-35000000.00"));
   ...
   Test.assertIsEqual(Number(tot_ZE), Number("-5000000.00"));

   var tot_ZF = calculate_tot_ZF(tot_ZD,tot_ZE);
   Test.assertIsEqual(Number(tot_ZF), Number("-40000000.00"));

   var tot_ZG = calculate_tot_ZG(tot_ZB,tot_ZC,tot_ZF);
   Test.assertIsEqual(Number(tot_ZG), Number("61070480.00"));

   var tot_ZH = calculate_tot_ZH(tot_ZG,calculate_ZA(banDoc, startDate, endDate));
   Test.assertIsEqual(Number(tot_ZH), Number("328932880.00"));
